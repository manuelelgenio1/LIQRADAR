...
- [x] **🔴 RANGO DEL MAPA ADAPTATIVO (fix definitivo 1h/4h/1d/1w)** — los rangos fijos por TF seguían sin ajustarse al movimiento REAL: en 1h/4h/1d/1w el precio se mueve más de lo que cubría el rango fijo y los clusters quedaban recortados/amontonados. Fix en `estimateLiquidationMap`: el eje ahora se deriva de la **acción de precio reciente** (último 45% de velas, donde dominan las posiciones por recencia) **más un margen** para las bandas de liquidación de los apalancamientos seleccionados (`min(maxLevDist, 6%)`). Se hace UNIÓN con el rango configurado (nunca más estrecho que antes), así 15m no empeora y 1h/4h/1d/1w se ensanchan a lo real. El heatmap 2D ya era adaptativo (usa hi/lo real de velas).
